...
import sys
import os
from plusplacer_taxtastic.__main__ import *
import shutil
import json
import time
import argparse
...
def main(args):
    ...
    if q_aln != "":
        ...
        ref_dict, q_dict = seperate(aln_dict, leaf_dict)
        #print("ref_dict", ref_dict)
        #print("q_dict", q_dict)
    ...
    add_edge_nbrs(tree)
    jplace["tree"] = newick_edge_tokens(tree)
    ...
    for name, seq in q_dict.items():
        ...
        if subtree_flag == 'h':
            y = find_closest_hamming(seq, ref_dict, n, frag_flag)
            labels = []
            for taxon in y:
               labels.append(leaf_dict[taxon].get_label())
            print(f'Closest sister taxon found: {y[0]}')
        else:
            y = find_closest_hamming(seq, ref_dict, 1, frag_flag)
            print(f'Closest sister taxon found: {y[0]}')
            print (f'{time.perf_counter() - t0} seconds finding closest leaf')
            node_y = leaf_dict[y[0]]
            if subtree_flag == 'n':
                labels = subtree_nodes(tree, node_y, n)
            else:
                labels = subtree_nodes_with_edge_length(tree, node_y, n)
